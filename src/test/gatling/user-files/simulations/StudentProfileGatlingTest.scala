...
/**
 * Performance test for the StudentProfile entity.
 */
class StudentProfileGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the StudentProfile entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all studentProfiles")
            ...
            .body(StringBody("""{
                ...
                , "mailingAddress":null
                , "profilePhoto":null
                , "extendedCompletionDate":"2020-01-01T00:00:00.000Z"
                }""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_studentProfile_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created studentProfile")
                .get("${new_studentProfile_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created studentProfile")
            .delete("${new_studentProfile_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
