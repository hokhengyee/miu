...
public class StudentPaymentResourceIntTest {
	...
	public static StudentPayment createEntity(EntityManager em) {
		StudentPayment studentPayment = new StudentPayment().createdDate(DEFAULT_CREATED_DATE).amount(DEFAULT_AMOUNT)
				.description(DEFAULT_DESCRIPTION).paymentDate(DEFAULT_PAYMENT_DATE).paid(DEFAULT_PAID);
		// Add required entity
	...
	public void createStudentPayment() throws Exception {
		...
		restStudentPaymentMockMvc.perform(post("/api/student-payments").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(studentPayment))).andExpect(status().isCreated());
	...
	public void createStudentPaymentWithExistingId() throws Exception {
		...
		// must fail
		restStudentPaymentMockMvc
				.perform(post("/api/student-payments").contentType(TestUtil.APPLICATION_JSON_UTF8)
	...
	public void checkAmountIsRequired() throws Exception {
		...
		restStudentPaymentMockMvc.perform(post("/api/student-payments").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(studentPayment))).andExpect(status().isBadRequest());
	...
	public void getAllStudentPayments() throws Exception {
		...
		restStudentPaymentMockMvc.perform(get("/api/student-payments?sort=id,desc")).andExpect(status().isOk())
	...
	public void updateStudentPayment() throws Exception {
		...
		updatedStudentPayment.createdDate(UPDATED_CREATED_DATE).amount(UPDATED_AMOUNT).description(UPDATED_DESCRIPTION)
				.paymentDate(UPDATED_PAYMENT_DATE).paid(UPDATED_PAID);

		restStudentPaymentMockMvc.perform(put("/api/student-payments").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(updatedStudentPayment))).andExpect(status().isOk());
	...
	public void deleteStudentPayment() throws Exception {
		...
		// Get the studentPayment
		restStudentPaymentMockMvc.perform(
				delete("/api/student-payments/{id}", studentPayment.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
