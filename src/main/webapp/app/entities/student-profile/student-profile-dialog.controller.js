(function() {
	...
	angular.module('miuApp').controller('StudentProfileDialogController',
			StudentProfileDialogController);

	StudentProfileDialogController.$inject = [ '$timeout', '$scope',
			'$stateParams', '$uibModalInstance', 'DataUtils', 'entity',
			'StudentProfile', 'Salutation', 'Gender', 'User', 'LecturerUser',
			'StudentUser' ];
	...
	function StudentProfileDialogController($timeout, $scope, $stateParams,
			...
			Gender, User, LecturerUser, StudentUser) {
		var vm = this;
		...
		function save() {
			...
			if (vm.studentProfile.id !== null) {
				StudentProfile.update(vm.studentProfile, onSaveSuccess,
						onSaveError);
			} else {
				StudentProfile.save(vm.studentProfile, onSaveSuccess,
						onSaveError);
			}
		}
