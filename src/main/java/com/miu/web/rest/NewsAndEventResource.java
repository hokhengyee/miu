...
public class NewsAndEventResource {
	 ...
	 *
	 * @param newsAndEvent
	 *            the newsAndEvent to create
	 * @return the ResponseEntity with status 201 (Created) and with body the
	 *         new newsAndEvent, or with status 400 (Bad Request) if the
	 *         newsAndEvent has already an ID
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	@PostMapping("/news-and-events")
	@Timed
	public ResponseEntity<NewsAndEvent> createNewsAndEvent(@Valid @RequestBody NewsAndEvent newsAndEvent)
			throws URISyntaxException {
		LOGGER.debug("REST request to save NewsAndEvent : {}", newsAndEvent);

		if (newsAndEvent.getId() != null) {
			return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("newsAndEvent", "idexists",
					"A new newsAndEvent cannot already have an ID")).body(null);
		}
		...
		return ResponseEntity.created(new URI("/api/news-and-events/" + result.getId()))
				.headers(HeaderUtil.createEntityCreationAlert("newsAndEvent", result.getId().toString())).body(result);
	}
	 ...
	 *
	 * @param newsAndEvent
	 *            the newsAndEvent to update
	 * @return the ResponseEntity with status 200 (OK) and with body the updated
	 *         newsAndEvent, or with status 400 (Bad Request) if the
	 *         newsAndEvent is not valid, or with status 500 (Internal Server
	 *         Error) if the newsAndEvent couldnt be updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	@PutMapping("/news-and-events")
	@Timed
	public ResponseEntity<NewsAndEvent> updateNewsAndEvent(@Valid @RequestBody NewsAndEvent newsAndEvent)
			throws URISyntaxException {
	 ...
	 *
	 * @param id
	 *            the id of the newsAndEvent to retrieve
	 * @return the ResponseEntity with status 200 (OK) and with body the
	 *         newsAndEvent, or with status 404 (Not Found)
	 */
	 ...
	 *
	 * @param id
	 *            the id of the newsAndEvent to delete
