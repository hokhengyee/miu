...
public class EntryQualificationResource {
	...
	private final Logger LOGGER = LoggerFactory.getLogger(EntryQualificationResource.class);

	/**
	 * POST /entry-qualifications : Create a new entryQualification.
	 *
	 * @param entryQualification
	 *            the entryQualification to create
	 * @return the ResponseEntity with status 201 (Created) and with body the
	 *         new entryQualification, or with status 400 (Bad Request) if the
	 *         entryQualification has already an ID
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	@PostMapping("/entry-qualifications")
	@Timed
	public ResponseEntity<EntryQualification> createEntryQualification(
			@Valid @RequestBody EntryQualification entryQualification) throws URISyntaxException {
		LOGGER.debug("REST request to save EntryQualification : {}", entryQualification);
		if (entryQualification.getId() != null) {
			return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("entryQualification", "idexists",
					"A new entryQualification cannot already have an ID")).body(null);
		}
	 ...
	 *
	 * @param id
	 *            the id of the entryQualification to delete
	 * @return the ResponseEntity with status 200 (OK)
	 */
	@DeleteMapping("/entry-qualifications/{id}")
	@Timed
	public ResponseEntity<Void> deleteEntryQualification(@PathVariable Long id) {
		LOGGER.debug("REST request to delete EntryQualification : {}", id);
		entryQualificationRepository.delete(id);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert("entryQualification", id.toString()))
				.build();
	}
	...
	public ResponseEntity<List<EntryQualification>> getAllEntryQualifications(@ApiParam Pageable pageable)
			throws URISyntaxException {
		LOGGER.debug("REST request to get a page of EntryQualifications");
	 ...
	 *
	 * @param entryQualification
	 *            the entryQualification to update
	 * @return the ResponseEntity with status 200 (OK) and with body the updated
	 *         entryQualification, or with status 400 (Bad Request) if the
	 *         entryQualification is not valid, or with status 500 (Internal
	 *         Server Error) if the entryQualification couldnt be updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	@PutMapping("/entry-qualifications")
	@Timed
	public ResponseEntity<EntryQualification> updateEntryQualification(
			@Valid @RequestBody EntryQualification entryQualification) throws URISyntaxException {
		LOGGER.debug("REST request to update EntryQualification : {}", entryQualification);
		if (entryQualification.getId() == null) {
		...
		return ResponseEntity.ok()
				.headers(
						HeaderUtil.createEntityUpdateAlert("entryQualification", entryQualification.getId().toString()))
				.body(result);
	}
