...
public class StaticPageTypeResource {
    ...
    private static final String ENTITY_NAME = "staticPageType";

    private final StaticPageTypeRepository staticPageTypeRepository;

    public StaticPageTypeResource(StaticPageTypeRepository staticPageTypeRepository) {
        this.staticPageTypeRepository = staticPageTypeRepository;
    }
    ...
    public ResponseEntity<StaticPageType> createStaticPageType(@Valid @RequestBody StaticPageType staticPageType) throws URISyntaxException {
        ...
        if (staticPageType.getId() != null) {
            throw new BadRequestAlertException("A new staticPageType cannot already have an ID", ENTITY_NAME, "idexists");
        }
     ...
     * or with status 500 (Internal Server Error) if the staticPageType couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    ...
    public ResponseEntity<StaticPageType> updateStaticPageType(@Valid @RequestBody StaticPageType staticPageType) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, staticPageType.getId().toString()))
            .body(result);
    }
    ...
    @Timed
    public ResponseEntity<List<StaticPageType>> getAllStaticPageTypes(Pageable pageable) {
    ...
    public ResponseEntity<StaticPageType> getStaticPageType(@PathVariable Long id) {
        ...
        Optional<StaticPageType> staticPageType = staticPageTypeRepository.findById(id);
        return ResponseUtil.wrapOrNotFound(staticPageType);
    }
    ...
    public ResponseEntity<Void> deleteStaticPageType(@PathVariable Long id) {
        ...
        staticPageTypeRepository.deleteById(id);
