...
public class ResearchPaperResource {
    ...
    public ResponseEntity<ResearchPaper> createResearchPaper(@Valid @RequestBody ResearchPaper researchPaper) throws URISyntaxException {
        ...
        if (researchPaper.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("researchPaper", "idexists", "A new researchPaper cannot already have an ID")).body(null);
        }
        ...
        return ResponseEntity.created(new URI("/api/research-papers/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert("researchPaper", result.getId().toString()))
            .body(result);
    }
     ...
     * or with status 500 (Internal Server Error) if the researchPaper couldnt be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    ...
    public ResponseEntity<ResearchPaper> getResearchPaper(@PathVariable Long id) {
        ...
        return Optional.ofNullable(researchPaper)
    ...
    public ResponseEntity<Void> deleteResearchPaper(@PathVariable Long id) {
        ...
        return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert("researchPaper", id.toString())).build();
    }
